...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="systems-slides-footer-contract-") as temporary:
        ...
        for dpi in (72, 96, 144, 288):
            ...
            width, height, pixels = read_ppm(prefix.with_suffix(".ppm"))
            scale = dpi / 72
            if width != round(960 * scale) or height != round(PAGE_HEIGHT * scale):
                errors.append(f"{dpi} DPI raster has unexpected size {width}x{height}")
                continue

            bottom = {pixel(pixels, width, x, height - 1) for x in range(width)}
            if bottom != {PURPLE}:
                errors.append(f"{dpi} DPI Footer bottom row is not uniformly #402B63")
            ...
            if boundary is None or abs(boundary - FOOTER_Y) > 0.5:
                errors.append(
                    f"{dpi} DPI Footer begins at row {first_purple} ({boundary}pt), "
            ...
            for y in range(y0, y1):
                if any(all(channel >= 250 for channel in pixel(pixels, width, x, y)) for x in range(x0, x1)):
                    white_rows.append(y)
            if not white_rows:
                errors.append(f"{dpi} DPI symmetric Footer logo is not visible")
                continue
            visible_center = ((white_rows[0] + white_rows[-1] + 1) / 2) / scale
            if abs(visible_center - FOOTER_CENTER) > 0.5:
                errors.append(
                    f"{dpi} DPI Footer logo center is {visible_center:.3f}pt, "
    ...
    print(
        ...
        "the symmetric logo is centered within 0.5pt, and #402B63 reaches the bottom edge at 72/96/144/288 DPI."
    )
    return 0
